...
public class OptimalPathPlanner {
    ...
    private Path computeShortestPath(Position start, Position end, String pathType) {
        return switch (pathType) {
            case "S" -> computeStraightPath(start, end);

            case "L" -> computeLeftPath(start, end);

            case "R" -> computeRightPath(start, end);

            case "SR" -> computeSRPath(start, end);

            case "SL" -> computeSLPath(start, end);

            case "RL" -> computeRLPath(start, end);

            case "LR" -> computeLRPath(start, end);

            case "LS" -> computeLSPath(start, end);

            case "RS" -> computeRSPath(start, end);

            case "RSR" -> computeRSRPath(start, end);

            case "LSL" -> computeLSLPath(start, end);

            case "LSR" -> computeLSRPath(start, end);

            case "RSL" -> computeRSLPath(start, end);
...
//        Hm..In real word, it may not need to be exactly same theta for straight line to be the best move. Need a range of theta
        if (start.getTheta() != end.getTheta()) {
            return null;
        }
        ...
        moves.add(new Move(Move.MoveDir.S, distance));
...
//        Might need a range agn instead of exactly matching
        if (calculateStraightDistance(end, circleCentre) != RADIUS) {
            return null;
        }
...
//        Determine angle
        ...
        ArrayList<Move> moves = new ArrayList<Move>();
        moves.add(new Move(Move.MoveDir.L, arcLen));
        return new Path(moves);
    }
...
//        Determine angle
        ...
        double arcLen = calculateArcLength(angle);
        ArrayList<Move> moves = new ArrayList<Move>();
        moves.add(new Move(Move.MoveDir.R, arcLen));
        return new Path(moves);
    }
